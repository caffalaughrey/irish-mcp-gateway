...
pub async fn run() -> ExitCode {
    ...
    match cli.command {
        ...
        Commands::TestGrammar { url, text } => {
            match test_grammar(url, &text).await {
                Ok(_) => {
...
async fn show_status(url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🏥 Health Status: {}", 
        if health_response.status().is_success() { "✅ Healthy" } else { "❌ Unhealthy" });
    ...
    println!("\n📋 Configuration:");
    println!("  Mode: {}", std::env::var("MODE").unwrap_or_else(|_| "server".into()));
    println!("  Port: {}", std::env::var("PORT").unwrap_or_else(|_| "8080".into()));
    println!("  Log Level: {}", std::env::var("RUST_LOG").unwrap_or_else(|_| "info".into()));
...
async fn test_grammar(url: Option<String>, text: &str) -> Result<(), Box<dyn std::error::Error>> {
    let grammar_url = url.or_else(|| std::env::var("GRAMADOIR_BASE_URL").ok())
...
mod tests {
    ...
    async fn test_test_grammar_no_url() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("No grammar service URL"));
    }
